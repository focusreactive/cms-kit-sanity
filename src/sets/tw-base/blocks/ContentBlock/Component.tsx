...
// @ts-ignore
const sa: AdapterFn = (cmsProps: CMSProps) => {
  return {
    key: getCmsKey(cmsProps),
    title: cmsProps?.title,
    subtitle: cmsProps?.subtitle,
    description: cmsProps?.description,
    image: imageConverter(cmsProps?.image),
    points: cmsProps?.points?.map((point) => ({
      ...point,
      icon: imageConverter(point?.icon),
    })),
    customRichText1: cmsProps?.customRichText1,
  };
};

export default withCMS({ sa })(Content);
