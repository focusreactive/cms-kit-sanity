...
const defaultTeamMembers: TeamMember[] = [
  {
    ...
    socialLinks: [
      { href: '#', svg: <FacebookIcon /> },
      { href: '#', svg: <TwitterIcon /> },
      { href: '#', svg: <GitHubIcon /> },
      { href: '#', svg: <LinkedInIcon /> },
    ],
  },
  {
    name: 'Jese Leos',
...
const SocialIcon: React.FC<SocialIconProps> = ({ href, svg }) => (
  <li>
    <a
      href={href}
...
const teamCardAdapter: AdapterFn = (props) => {
  return {
    ...props,
    // @ts-ignore
    role: props.position,
    // @ts-ignore
    imageUrl: props.avatar?.url || 'https://flowbite.s3.amazonaws.com/blocks/marketing-ui/avatars/sofia-mcguire.png',
    // @ts-ignore
    description: props.bio,
  };
};

const TeamMemberCard: React.FC<TeamMember> = withCMS({ sa: teamCardAdapter })(
  ({
    name = 'John Dow',
    role = 'Developer',
    description = 'Bio',
    imageUrl = '',
    socialLinks = defaultTeamMembers[0].socialLinks,
  }) => (
    <div className="items-center bg-gray-50 rounded-lg shadow sm:flex dark:bg-gray-800 dark:border-gray-700">
      <a href="#">
        <img
          className="w-full rounded-lg sm:rounded-none sm:rounded-l-lg max-w-32"
          src={imageUrl}
          alt={`${name} Avatar`}
        />
      </a>
      <div className="p-5">
        ...
        <ul className="flex space-x-4 sm:mt-0">
          {socialLinks.map((link, index) => (
            <SocialIcon key={index} href={link.href} svg={link.svg} />
          ))}
        </ul>
      </div>
    </div>
...
const TeamSection: React.FC<TeamSectionProps> = ({
  ...
  // const { title, description, teamMembers } = defaultProps;
...
export default withCMS({ sa })(TeamSection);
